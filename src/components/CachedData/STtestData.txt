...
const allTravelsVing = [ArubaAllYears, TokyoAllYears, ParisAllYears];


const makeDataSet = (dataArr, color = 'rgb(60, 179, 113)') => {
  let datasetObj = {
...
const monthData = (arrCities) => {
  arrCities.forEach(arrCity => {
    ...
    let datasets = [];
    datasets.push(makeDataSet(avgTempData));
    datasets.push(makeDataSet(avgHumidityData));
    console.log(datasets)
    let dataSetArr = [];
    dataSetArr.push({ ...datasets })
    console.log(makeChartDataObject(dataSetArr, filteredMonthNum))

    // chartDataObject.labels = filteredMonthNum;
    // console.log(datasets)
...
const CollectedData = () => {
  ...
  return (
    <div>
      <Line
        data={monthData(allTravelsVing)}
        options={{
          responsive: true,
          title: { text: 'CityStats', display: true },
          scales: {
            yAxes: [
              {
                ticks: {
